...
The `setColorBasedOnCount` function then adjusts the colour of the number dyanaimcally, using a gradient from green (to indicate high inventory) to red (low inventory). These colours give users a visual cue about how many meals are left, further improving user experience. 
...
Finally, the `itialiseDashboard` function is used to call all functions, tieing everything together:
